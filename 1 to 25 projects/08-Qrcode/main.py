...
# - **Library Used:** `qrcode` and `opencv-python` (cv2)
# - **Purpose:**
#   - Generate QR codes with custom data.
#   - Decode QR codes from image files.
# - **Functions Implemented:**
#   - `create_qr_code(data, filename)` — Generates a QR code image with provided data.
#   - `decode_qr_code(filename)` — Decodes and extracts data from a QR code image.
